...
torch.backends.cudnn.benchmark = True # Optimize cuDNN for static network structure
...
# ---------------------------------------------------------
CONFIG = {
    "n_clients_list":  [5, 20, 50], # Will test all 3 scales
    "n_rounds":        100,
    "local_epochs":    3,
    "lr":              0.001,
    "weight_decay":    1e-4,
...
def local_train(global_model, loader, lr, weight_decay, local_epochs, clip_C):
    ...
    for _ in range(local_epochs):
        for X_b, y_b in loader:
            # Data is already on GPU, no need to send it again
            optimizer.zero_grad()
    ...
    # Calculate gradient delta and move to CPU for anomaly numpy processing
    grad = {}
...
def federated_round(global_model, client_loaders, trust_mgr, detectors, policy, round_t, byzantine_clients, attack_type, config):
    ...
    # Apply aggregated gradients to GPU model
    ...
    for name, param in global_model.named_parameters():
        ...
        for i, grad in grads_accepted.items():
            ...
            # Transfer numpy gradient to GPU for fast tensor addition
...
def evaluate(model, X_test, y_test, seq_len=24):
    ...
    # Process test set on GPU
    X_t = torch.tensor(X_seq, device=DEVICE)
    with torch.no_grad():
        logits = model(X_t)
        probs = torch.softmax(logits, dim=1)[:, 1].cpu().numpy() # Move predictions back to CPU for metrics
...
def run_experiment(n_clients, attack_type, byz_fraction, seed, config):
    ...
    # Dataloader creation with GPU Pre-loading
    client_loaders = []
    for X, y in client_data:
        X_n = scaler.transform(X)
        X_seq, y_seq = make_sequences(X_n, y, config["seq_len"], seed=seed)
        
        # We push the tensors directly to VRAM here
...
def main():
    ...
    print("MetaTrust-FL Scalability Experiment (GPU Optimized)")
    print("=" * 60)
    ...
    for n_clients in CONFIG["n_clients_list"]:
        ...
        for attack in attack_types:
            ...
            for seed in range(CONFIG["n_seeds"]):
                ...
                print(f"  N={n_clients} attack={attack:10s} seed={seed}  AUROC={r['auroc']:.3f}  det={r['detection_rate']:.1%}  t={r['avg_proof_time']:.3f}s  (Real Time: {r['wall_clock_min']:.2f}m)")
    ...
    with open("scalability_results_gpu.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\nSaved: scalability_results_gpu.json")
